...
class Database():
    def __init__(self):
        self.connection = sqlite3.connect('rfid_database.db')
    ...
    def get(self, filter: UserDataFilter = UserDataFilter()):
        ...
        self.cursor.execute(
            query, (rfid, full_name, is_enter, dateFrom, dateFrom, dateTo, dateTo))
        return self.cursor.fetchall()

    def create(self, data: UserData) -> None:
        self.cursor.execute("""
        INSERT INTO users (rfid, full_name, is_enter, date) VALUES (?, ?, ?, ?)
        """, (data["rfid"], data["full_name"], data["is_enter"], int(data["date"].timestamp())))
